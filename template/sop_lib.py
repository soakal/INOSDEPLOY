...
class SOPBuilder:
    ...
    def bullet_rich(
        ...
        *,
        ...
        """Bullet built from (text, bold) tuples. Use bold=True for UI element names."""
        p = self._bullet_paragraph(sub)
    ...
    # label color + shading by kind (red=danger, orange=advisory, blue=info)
    _CALLOUT_STYLES = {
        "WARNING":   (RGBColor(0xC0, 0x00, 0x00), "FCE4E4"),
        "IMPORTANT": (RGBColor(0xC5, 0x5A, 0x11), "FCE9D9"),
        "TIP":       (RGBColor(0xC5, 0x5A, 0x11), "FCE9D9"),
        "NOTE":      (RGBColor(0x1F, 0x4E, 0x79), "DEEAF6"),
        "INFO":      (RGBColor(0x1F, 0x4E, 0x79), "DEEAF6"),
    }

    def callout(self, kind: str, text: str):
        """Visually-distinct callout: bold colored label + body, indented and shaded.

        kind is one of WARNING / IMPORTANT / TIP / NOTE / INFO (case-insensitive).
        Renders a uniform block so each kind looks the same everywhere it appears.
        """
        key = kind.strip().upper()
        color, shade = self._CALLOUT_STYLES.get(key, self._CALLOUT_STYLES["NOTE"])
        p = self.doc.add_paragraph()
